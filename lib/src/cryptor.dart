...
class _Cryptor {
  ...
  Future<Uint8List> decryptDataFromFile(String srcFilePath) async {
    ...
    final int file_size_modulo = await (_readChunkInt(f, 1, 'file size modulo') as FutureOr<int>);
    _log('FILE SIZE MODULO', file_size_modulo);
    if (file_size_modulo < 0 || file_size_modulo >= 16) {
      throw AesCryptDataException('Invalid file size modulo: $file_size_modulo');
    }
  ...
  Future<Map<_Data,Uint8List>> _readKeys(RandomAccessFile f) async {
    ...
    if (version_chunk == 2) {
      ...
      while (ext_length != 0) {
        ...
        ext_length = await (_readChunkInt(f, 2, 'extension length') as FutureOr<int>);
      }
    }
